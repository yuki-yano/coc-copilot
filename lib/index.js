...
var activate = async (context) => {
  const extensionConfig = import_coc.workspace.getConfiguration(sourceName);
  const isEnable = extensionConfig.get("copilot.enable", true);
  const kindLabel = extensionConfig.get("kindLabel", "\uF113\xA0");
  const priority = extensionConfig.get("copilot.priority", 1e3);
  if (!isEnable) {
    return;
  }
...
var getCompletionItems = async (option, kindLabel, priority) => {
